...
def get_num_attempts():
    """Get user to input number of incorrect attempts for the game."""
    while True:
...
def get_max_word_length():
	"""Get useR to input minimum word length for the game."""
	while True:
		max_word_length = input('What max word length do you want? [4-12] ')
		try:
			max_word_length = int(max_word_length)
			if (max_word_length >= 4 and max_word_length <= 12):
				return max_word_length             
			else:                 
				print('{0} is not between 4 and 16'.format(max_word_length))         
		except ValueError:             
			print('{0} is not an integer between 4 and 16'.format( max_word_length)) 
...
def get_next_letter(remaining_letters):     
	"""Get the user to input next letter."""
	x=0
...
def play_hangman():
	...
	word =get_random_word(max_word_length)     
	print()     
